...
namespace CreatorKitCode 
{
    ...
    public class Weapon : EquipmentItem
    {
        ...
        public class AttackData
        {
            ...
            public int AddDamage(StatSystem.DamageType damageType, int amount)
            {
                ...
                if (damageType == StatSystem.DamageType.Physical)
                {
                    ...
                    if(m_Source !=  null)
                        addedAmount += Mathf.FloorToInt(addedAmount * m_Source.damageableBehaviour.Stats.stats.strength * 0.01f);
                
                    //each poitn of defense remove 1 damage, with a minimum of 1 damage
                    addedAmount = Mathf.Max(addedAmount - m_Target.damageableBehaviour.Stats.stats.defense, 1);
                }
            
                //we then add boost per damage type. Not this is called elementalBoost, but physical can also be boosted
                if(m_Source != null)
                    addedAmount += addedAmount * Mathf.FloorToInt(m_Source.damageableBehaviour.Stats.stats.elementalBoosts[(int)damageType] / 100.0f);
            
                //Then the elemental protection that is a percentage
                addedAmount -= addedAmount * Mathf.FloorToInt(m_Target.damageableBehaviour.Stats.stats.elementalProtection[(int)damageType] / 100.0f);
        ...
        public void Attack(CharacterData attacker, CharacterData target)
        {
            ...
            target.Damage(attackData.GetFullDamage());
...
public class WeaponEditor : Editor
{
    ...
    static public void CreateWeapon()
    {
        ...
        newWeapon.Slot = (GlobalEnum.EquipmentSlot)666;
